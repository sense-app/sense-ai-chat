...
const LearningsSchema = z.object({
    ...
    followUpQuestions: z.array(z.string()).describe('Follow up questions to search online'),
    products: z.array(productSchema).describe('Products that best matches the user query'),
});
export type Learnings = z.infer<typeof LearningsSchema>;

const getPrompt = (userQuery: string, questions: string[], contents: string[]) => {
    return `
    Your are an advanced shopping assistant. 
    The user has asked you for <query>${userQuery}</query>.
    ${questions?.length > 0 ? `To understand the user's query, you have breakdown the following questions: 
    ${questions.join(', ')}.` : ''}
