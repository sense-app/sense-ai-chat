...
export const excludeActions = (excludeList: Action[]): Set<Action> => {
  return new Set(
    [...allActions].filter((action) => !excludeList.includes(action))
  );
};
...
const actionMap: Record<Action, string> = {
  ...
  [Action.Search]:
    "Query external sources using a public search engine. Focus on solving one specific aspect of the question. Only give keywords search query, not full sentences",
  [Action.Read]:
    "Visit any URLs from the available URLs to get more information. Identify key learnings and metrics from the content. Aggregate the products from different stores into structured json format. Include as many details as possible."
...
export const productSchema = z.object({
  ...
  imageUrl: z
    .string()
    .describe(
      "URL of the product image from the e-commerce store where this product can be purchased"
    ),
  category: z.string(),
  review: z.string(),
  ...
  store: z
    .object({
      ...
      imageUrl: z
        .string()
        .describe(
          "URL of the store image. The image url should come from the e-commerce store"
        ),
      shopUrl: z
        .string()
        .describe(
...
export const SHOPPING_SYSTEM_PROMPT = `
    ...
    Do not deviate and answer any questions without using the available tools and actions provided to you.

    You have the following tools at your disposal:
    reflect - To Breakdown user's intent and questions to smaller shopping related questions that you can first find the answer.
...
export const getPrompt = (knowledgeBank: KnowledgeBank) => {
  ...
  if (searchResults.length > 0) {
    sections.push(`## Search Results
            ...
            ${searchResults
              .map(
                (result) =>
                  `URL: ${result.url}\t title: ${result.title}\t description: ${result.description}\t icon: ${result.icon} `
              )
              .join("\n")}`);
  }
  ...
  if (availableActions) {
    sections.push(`## Available Actions
            ...
            ${Array.from(availableActions)
              .map((action) => `${Action[action]} - ${actionMap[action]}`)
              .join("\n")}`);
  }
