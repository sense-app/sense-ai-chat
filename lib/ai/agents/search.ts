...
import { SafeSearchType, SearchResults, search as duckduckgoSearch} from "duck-duck-scrape";
    ...
    parameters: z.object({
      ...
      execute: async(params) => {
        ...
        for (const query of seachQueries.queries) {
            try {
                const result = await duckduckgoSearch(query, { safeSearch: SafeSearchType.STRICT });
                results.push(result);
            } catch (error) {
                console.error(error);
            }
